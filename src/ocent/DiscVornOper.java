...
public class DiscVornOper implements TestBedOperation, Globals {
  ...
  plotstd
  effdiscs labeledges _ _  test perturb _ _ newcontainment
  stdk guark _ _ _ bisectors minmaxk
  plotposs psite _ subsetcolor _ plotstored store
  */
    ...
    private static final int NEWCONTAINMENT   = 1521;//!
    private static final int STDK             = 1522;//!
    private static final int GUARK            = 1523;//!
    private static final int BISECTORS        = 1527;//!
    private static final int MINMAXK          = 1528;//!
  ...
  public void addControls() {
    ...
    C.sStaticText("Plots guaranteed Voronoi diagram for uncertain discs");
    {
      C.sOpen();
      {
        ...
        C.sNewColumn();
        C.sIntSpinner(STDK, "k:", "Order-k value for standard Voronoi diagram",
            1, 5, 1, 1);
...

      {
        ...
        C.sHide();
        C.sIntSpinner(MINMAXK, "k:", "Order-k value for MinMax diagram", 1, 5,
            1, 1);
        C.sClose();
      }

      {
        ...
        C.sHide();
        C.sCheckBox(BISECTORS, "Bisectors", "Plot site bisectors", false);
        C.sClose();
      }
      {
        C.sOpen();
...
//        C.sNewColumn();
  ...
  private void buildPossibleCell() {
    EdDisc[] circ = OCentMain.getDiscs();
    int s = C.vi(PSITE);
    if (s < 10) {
      if (s < circ.length) {
...
//    if (C.vb(PLOTCRUST)) {
//      try {
//        DArray a = Z_CrustOper.build(Main.getDiscs());
//        crust = a.getDArray(0);
//      } catch (FPError e) {
...
//  private DArray crust;
  ...
  public void paintView() {
    ...
    if (C.vb(PLOTSTORED) && savedGraph != null) {
    ...
    if (C.vb(SUBSETS)) {
      ...
      Hyperbola[] hypList = VornUtil.buildSubset(circ, !C.vb(NOCLIP),
          SubsetDiscBisector.S);

      VornGraph vg = new VornGraph(hypList);

      boolean cf = C.vb(SUBSETCOLOR);
    ...
    if (C.vb(PLOTGUAR)) {
      Hyperbola[] edges = VornUtil.build(circ, kGuar, GuaranteedDiscBisector.S,0);
...
//    }
//    if (crust != null)
//      Z_CrustOper.plot(Main.getDiscs(), crust);
  ...
  private void constructMinMax() {
    ...
    Hyperbola[] vDiag = VornUtil.build(filterContained(circ), 
        k, StandardDiscBisector.S,0);
    VornGraph g = new VornGraph(vDiag);
...
//    if (spines == null) {
//      spines = Z_Spine.construct(Main.getDiscs());
//    }
//    return spines;
//  }
